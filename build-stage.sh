...
rm -f /tmp/priv.key
export PIPELINE_IMAGE_URL="us.icr.io/tylerlisnamespace/hello-world-azure-app:${GIT_HEAD_COMMIT}"
export FULL_IMAGE_TAG="$PIPELINE_IMAGE_URL"
make dockerbuild
envsubst < deployment/deployment.yaml > deployment/templated-deployment.yaml
ibmcloud sat config version create --name "$GIT_HEAD_COMMIT" --config hello-world-azure-app --file-format yaml --read-config deployment/templated-deployment.yaml
